 ...
 *  3. the dev default `seed-interviewer` (matches the seed script + expert dev-token fallback).
 *
 * The expert socket accepts this token as a raw user id when Supabase env is unset.
 */
export function AuthProvider({ children }: { children: ReactNode }) {
  const [token, setToken] = useState<string>("seed-interviewer");

  useEffect(() => {
    const fromUrl = new URLSearchParams(window.location.search).get("token");
    const stored = window.localStorage.getItem("probe-token");
    const resolved = fromUrl || stored || "seed-interviewer";
